...
for line in io.lines("linhas.txt") do
  ...
  linha.nome[counter] = string.sub(linhas[counter],pos+1, tamanho-1)
  counter = counter  + 1
end
...
for line in io.lines("horarios.txt") do
  ...
  tamanho = string.len(linhas[counter])
  pos = string.find(linhas[counter], ';')
  pos1 = string.find(linhas[counter], ';', pos+1)
  --pos2 = string.find(linhas[counter], ';', pos1+1)
  rota.id[counter] = string.sub(linhas[counter],i, pos-1)
  rota.idl[counter] = string.sub(linhas[counter],pos1-1,pos1-1)
  --io.write(rota.idl[counter])
  rota.idp[counter] = string.sub(linhas[counter], pos1+1, tamanho-1)
  counter = counter  + 1
end
...
function LuaNCL (evt)
  if evt.class =='ncl' and evt.type == 'presentation' and evt.action == 'start' then
    ...
    canvas:attrColor ('black')
    canvas:attrFont ('arial', 16, 'bold')
    canvas:drawText (30,y,"Horarios")
